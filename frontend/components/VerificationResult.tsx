...
export function VerificationResult({ verification }: { verification: Verification | null }) {
  ...
  const commitUrl = `${verification.repository_url}/commit/${verification.commit_sha}`;
  const releaseUrl = verification.release_ref ? `${verification.repository_url}/releases/tag/${encodeURIComponent(verification.release_ref)}` : "";
  return (
    <section className="rounded-2xl border border-border bg-card/60 p-6">
      ...
      <div className="mt-4"><p className="text-xs uppercase tracking-wide text-muted-foreground">Reviewed immutable commit</p><a className="mt-1 inline-flex items-center gap-1 break-all font-mono text-sm text-accent hover:underline" href={commitUrl} target="_blank" rel="noreferrer">{verification.commit_sha}<ExternalLink className="h-3.5 w-3.5 shrink-0" /></a></div>
      <div className="mt-4"><p className="text-xs uppercase tracking-wide text-muted-foreground">Release reference</p>{releaseUrl ? <a className="mt-1 inline-flex items-center gap-1 break-all text-sm text-accent hover:underline" href={releaseUrl} target="_blank" rel="noreferrer">{verification.release_ref}<ExternalLink className="h-3.5 w-3.5 shrink-0" /></a> : <p className="mt-1 text-sm">Not supplied</p>}</div>

...
export class ReleaseProofContract {
  ...
  async getVerification(id: string): Promise<Verification> {
    ...
    return {
      id: stringOf(raw.id), creator: stringOf(raw.creator), repository_url: stringOf(raw.repository_url),
  ...
  createVerification(input: CreateVerificationInput, listener?: LifecycleListener) {
    return this.write("create_verification", [input.verification_id, input.repository_url, input.release_ref, input.criteria], listener);
  }
